...
public interface RoomRepository extends JpaRepository<Room, Long> {
    @Query("select new com.edu.fpt.hoursemanager.management.room.model.response.GetRoomResponse(r.id,r.codeTypeRoom, r.name) from Room r where r.id = :id and r.deleted = false")
    GetRoomResponse checkExist(@Param("id") String id);

    @Query("select new com.edu.fpt.hoursemanager.management.room.model.response.GetRoomBuildingResponse(r.id,tr.id, tr.capacity,r.name, b.id, b.name, c.id) " +
    ...
    @Query("select new com.edu.fpt.hoursemanager.management.room.model.response.GetRoomResponse(r.id,r.codeTypeRoom, r.name) from Room r where r.deleted = false")
    List<GetRoomResponse> getAllRoom();

    @Query("select new com.edu.fpt.hoursemanager.management.room.model.response.GetRoomResponse(r.id,r.codeTypeRoom, r.name) from Room r where r.name LIKE %:name% and r.deleted = false ")
