...
public interface VehicleManageRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query("select new com.edu.fpt.hoursemanager.management.contract.model.response.VehicleResponse (v.id, v.name, v.color, v.numberPlate, v.type, r.id)" + 
    " from Vehicle v inner join v.contract vc inner join vc.renterRooms rr inner join rr.room r where v.deleted = false and r.id = :id")
    List<VehicleResponse> getAllVehicleByRoomId(@Param("id") Long id);
